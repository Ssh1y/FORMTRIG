...
int main(void) {
  ...
  if (rec->magic != FORMTRIG_SHM_MAGIC ||
    ...
    return 3;
  if (rec->component_count < 3 || rec->atom_signal_count < 1) return 4;
  if (rec->components[0].role != FORMTRIG_ROLE_ROOT_OBSERVE) return 5;
  if (rec->components[1].role != FORMTRIG_ROLE_DESIRED_PRODUCER) return 6;
  if (rec->components[2].role != FORMTRIG_ROLE_USE) return 7;
  ...
  if (!has_role(signal, FORMTRIG_ROLE_ROOT_OBSERVE) ||
    ...
    return 8;
  if ((signal->producer_bits & 2u) == 0) return 9;
  if ((signal->use_bits & 2u) == 0) return 10;
