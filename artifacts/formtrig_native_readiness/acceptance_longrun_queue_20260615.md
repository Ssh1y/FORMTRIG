...
Machine-readable triage is generated by:
...
Current triage result: no target in this queue is a main hard SOTA-gap case.
`PNG006` and `LIBCOAP_CVE_2023_35862` are demoted to control/negative evidence
because matched faithful baselines also trigger. `LIBXML2_1107` is demoted
because the harness exposes the terminal trigger as a direct input knob. The
next long-run budget should therefore go to hard-target discovery and triage,
not additional primary-claim runs on these three targets.

Machine-readable hard-target discovery is generated by:
...
- first real-CVE replacements: `GPAC_3403` and `LIBARCHIVE_2936`;
- first Magma short-triage target with BindingSpec inputs: `PNG007`;
- first Magma BindingSpec-first candidates: `PDF003`, `SSL011`, `SSL015`,
  and `TIF012`;
- demoted controls retained outside the main queue: `PNG006`,
  `LIBCOAP_CVE_2023_35862`, and `LIBXML2_1107`.

Real-CVE execution readiness is generated by:
