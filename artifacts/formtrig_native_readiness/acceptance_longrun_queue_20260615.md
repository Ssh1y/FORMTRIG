...
the faithful baselines also trigger. If those baselines also trigger but
FORMTRIG reaches first `_T` earlier, the package is treated as speedup evidence
rather than baseline-impossibility evidence.
...
- keep easy/baseline-visible targets as control/native-readiness evidence
  unless FORMTRIG shows a same-budget first-`_T`/TTE speedup that justifies
  repetitions;
