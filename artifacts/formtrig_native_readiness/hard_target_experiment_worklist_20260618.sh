...
# benefit: Before comparing performance, prove that the candidate BindingSpec creates replay-stable pre-trigger guidance.
# blocked: pre-trigger lift guidance is not ready; terminal signal appeared without non-trigger guidance; semantic BindingSpec roles varied, but no accepted non-trigger frontier progress was observed
